...
def validate_metric(test_num, name, db_comp_query, tab_query, value_key, tolerance=0.01):
...
def validate_top_entity(test_num, name, db_comp_query, tab_query, entity_key, value_key, tolerance=0.01):
...
def test_database_comparison_complete_validation():
    ...
    Total: 49 queries (some use simplified validation)
    """
    ...
    test_num += 1
    if validate_metric(test_num, "School Fundraising",
    ...
    # 2. School Minutes
    test_num += 1
    if validate_metric(test_num, "School Minutes",
    ...
    # 3. School Sponsors
    test_num += 1
    if validate_metric(test_num, "School Sponsors",
    ...
    test_num += 1
    if validate_metric(test_num, "School Participation",
    ...
    # 5. School Size
    test_num += 1
    if validate_metric(test_num, "School Size",
    ...
    test_num += 1
    if validate_top_entity(test_num, "Student Top Fundraiser",
    ...
    test_num += 1
    if validate_top_entity(test_num, "Student Top Sponsors",
    ...
    for metric, value_key, tab_query in [
        ...
        test_num += 1
        if validate_top_entity(test_num, f"Team {metric.title()}",
    ...
    for metric, value_key, tab_query in [
        ...
        test_num += 1
        if validate_top_entity(test_num, f"Grade {metric.title()}",
    ...
    for metric, value_key, tab_query in [
        ...
        test_num += 1
        if validate_top_entity(test_num, f"Class {metric.title()}",
    ...
    assert failed == 0, f"{failed} queries failed (see output above for details)"
