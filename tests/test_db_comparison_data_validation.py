...
def validate_values(name, db_comp_value, tab_value, tolerance=0.01):
...
def test_database_comparison_data_validation():
    """
    Validate database comparison query results against actual tab data.
    ...
    """)
    if validate_values("Total Fundraising", db_comp['total_fundraising'], school_tab['total_fundraising']):
        passed += 1
    else:
        failed += 1
    ...
    """)
    if validate_values("Total Minutes", db_comp['total_minutes'], school_tab['total_minutes']):
        passed += 1
    else:
        failed += 1
    ...
    school_tab = get_query_result("""
        ...
        FROM Reader_Cumulative rc
    """)
    if validate_values("Total Sponsors", db_comp['total_sponsors'], school_tab['total_sponsors']):
        passed += 1
    else:
        failed += 1
    ...
    """)
    if validate_values("Participation %", db_comp['participation_pct'], school_tab['participation_pct'], tolerance=0.1):
        passed += 1
    else:
        failed += 1
    ...
    if db_comp and teams_tab:
        match = True
        if not validate_values("Team Name", db_comp['team_name'], teams_tab['team_name']):
            match = False
        if not validate_values("Fundraising", db_comp['total_fundraising'], teams_tab['total_fundraising']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and teams_tab:
        ...
        if not validate_values("Team Name", db_comp['team_name'], teams_tab['team_name']):
            match = False
        if not validate_values("Minutes", db_comp['total_minutes'], teams_tab['total_minutes']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and grade_tab:
        match = True
        if not validate_values("Grade Level", db_comp['grade_level'], grade_tab['grade_level']):
            match = False
        if not validate_values("Fundraising", db_comp['total_fundraising'], grade_tab['total_fundraising']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and grade_tab:
        ...
        if not validate_values("Grade Level", db_comp['grade_level'], grade_tab['grade_level']):
            match = False
        if not validate_values("Minutes", db_comp['total_minutes'], grade_tab['total_minutes']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and class_tab:
        match = True
        if not validate_values("Class Name", db_comp['class_name'], class_tab['class_name']):
            match = False
        if not validate_values("Fundraising", db_comp['total_fundraising'], class_tab['total_fundraising']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and class_tab:
        ...
        if not validate_values("Class Name", db_comp['class_name'], class_tab['class_name']):
            match = False
        if not validate_values("Minutes", db_comp['total_minutes'], class_tab['total_minutes']):
            match = False
        if match:
            passed += 1
        else:
    ...
    if db_comp and student_tab:
        match = True
        if not validate_values("Student Name", db_comp['student_name'], student_tab['student_name']):
            match = False
        if not validate_values("Fundraising", db_comp['fundraising'], student_tab['fundraising']):
            match = False
        if match:
            passed += 1
        else:
    ...
    assert failed == 0, f"{failed} comparisons failed (see output above for details)"
