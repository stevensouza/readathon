...
def validate_metric(num, name, db_comp_query, ground_truth_query, value_key, tolerance=0.01):
...
def validate_top_winner(num, name, db_comp_query, ground_truth_query, entity_key, value_key, tolerance=0.01):
    """Validate queries that return top winner (handles ties)"""
...
def test_all_49_database_comparison_queries():
    ...
    Total: 49 queries
    """
    failures = []  # Collect detailed failure info for pytest assertion
    ...
    if validate_metric(1, "School Fundraising", get_db_comparison_school_fundraising(),
        "SELECT COALESCE(SUM(donation_amount), 0) as total_fundraising FROM Reader_Cumulative",
        'total_fundraising'):
        passed += 1
    else:
        failed += 1
        failures.append("Query 1: School Fundraising")

    if validate_metric(2, "School Minutes", get_db_comparison_school_minutes(),
        ...
        failed += 1
        failures.append("Query 2: School Minutes")

    if validate_metric(3, "School Sponsors", get_db_comparison_school_sponsors(),
        "SELECT COALESCE(SUM(sponsors), 0) as total_sponsors FROM Reader_Cumulative",
        'total_sponsors'):
        passed += 1
    else:
        failed += 1
        failures.append("Query 3: School Sponsors")

    if validate_metric(4, "School Participation", get_db_comparison_school_participation(),
        ...
        failed += 1
        failures.append("Query 4: School Participation")

    if validate_metric(5, "School Size", get_db_comparison_school_size(),
        "SELECT COUNT(DISTINCT student_name) as student_count FROM Roster",
        'student_count'):
        passed += 1
    else:
        failed += 1
        failures.append("Query 5: School Size")

    if validate_metric(6, "School Avg Participation", get_db_comparison_school_avg_participation(),
        ...
        failed += 1
        failures.append("Query 6: School Avg Participation")

    if validate_metric(7, "School Goal Met", get_db_comparison_school_goal_met(),
    ...
    if validate_metric(8, "School All Days Active", get_db_comparison_school_all_days_active(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
           AllDaysStudents AS (
    ...
    if validate_metric(9, "School Goal Met All Days", get_db_comparison_school_goal_met_all_days(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_metric(10, "School Color War Points", get_db_comparison_school_color_war_points(),
        """WITH BasePoints AS (
    ...
    if validate_top_winner(11, "Student Top Fundraiser", get_db_comparison_student_top_fundraiser(),
    ...
    if validate_top_winner(12, "Student Top Reader", get_db_comparison_student_top_reader(),
    ...
    if validate_top_winner(13, "Student Top Sponsors", get_db_comparison_student_top_sponsors(),
    ...
    if validate_top_winner(14, "Student Top Participation", get_db_comparison_student_top_participation(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(15, "Student Goal Met (Days)", get_db_comparison_student_goal_met(),
        """WITH StudentGoals AS (
    ...
    if validate_top_winner(18, "Student Avg Minutes/Day", get_db_comparison_student_avg_minutes_per_day(),
    ...
    if validate_top_winner(19, "Student Total Days", get_db_comparison_student_total_days(),
    ...
    if validate_top_winner(20, "Team Fundraising", get_db_comparison_team_top('fundraising'),
        ...
        failed += 1
        failures.append("Query 7: Team Fundraising")

    if validate_top_winner(21, "Team Minutes", get_db_comparison_team_top('minutes'),
        ...
        failed += 1
        failures.append("Query 8: Team Minutes")

    if validate_top_winner(22, "Team Size", get_db_comparison_team_top('size'),
        ...
        failed += 1
        failures.append("Query 9: Team Size")

    if validate_top_winner(23, "Team Sponsors", get_db_comparison_team_sponsors(),
        ...
        failed += 1
        failures.append("Query 10: Team Sponsors")

    if validate_top_winner(24, "Team Participation", get_db_comparison_team_participation(),
        ...
        failed += 1
        failures.append("Query 11: Team Participation")

    if validate_top_winner(25, "Team Avg Participation", get_db_comparison_team_avg_participation(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
           TeamBonusData AS (
    ...
    if validate_top_winner(26, "Team Goal Met", get_db_comparison_team_goal_met(),
    ...
    if validate_top_winner(27, "Team All Days Active", get_db_comparison_team_all_days_active(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(28, "Team Goal Met All Days", get_db_comparison_team_goal_met_all_days(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(29, "Team Color War Points", get_db_comparison_team_color_war_points(),
    ...
    if validate_top_winner(30, "Grade Fundraising", get_db_comparison_grade_top('fundraising'),
        ...
        failed += 1
        failures.append("Query 12: Grade Fundraising")

    if validate_top_winner(31, "Grade Minutes", get_db_comparison_grade_top('minutes'),
        ...
        failed += 1
        failures.append("Query 13: Grade Minutes")

    if validate_top_winner(32, "Grade Size", get_db_comparison_grade_top('size'),
        ...
        failed += 1
        failures.append("Query 14: Grade Size")

    if validate_top_winner(33, "Grade Sponsors", get_db_comparison_grade_sponsors(),
        ...
        failed += 1
        failures.append("Query 15: Grade Sponsors")

    if validate_top_winner(34, "Grade Participation", get_db_comparison_grade_participation(),
        ...
        failed += 1
        failures.append("Query 16: Grade Participation")

    if validate_top_winner(35, "Grade Avg Participation", get_db_comparison_grade_avg_participation(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
           GradeBonusData AS (
    ...
    if validate_top_winner(36, "Grade Goal Met", get_db_comparison_grade_goal_met(),
    ...
    if validate_top_winner(37, "Grade All Days Active", get_db_comparison_grade_all_days_active(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(38, "Grade Goal Met All Days", get_db_comparison_grade_goal_met_all_days(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(39, "Grade Color War Points", get_db_comparison_grade_color_war_points(),
    ...
    if validate_top_winner(40, "Class Fundraising", get_db_comparison_class_top('fundraising'),
        ...
        failed += 1
        failures.append("Query 17: Class Fundraising")

    if validate_top_winner(41, "Class Minutes", get_db_comparison_class_top('minutes'),
        ...
        failed += 1
        failures.append("Query 18: Class Minutes")

    if validate_top_winner(42, "Class Sponsors", get_db_comparison_class_top('sponsors'),
        ...
        failed += 1
        failures.append("Query 19: Class Sponsors")

    if validate_top_winner(43, "Class Participation", get_db_comparison_class_participation(),
    ...
    if validate_top_winner(44, "Class Size", get_db_comparison_class_top('size'),
        ...
        failed += 1
        failures.append("Query 20: Class Size")

    if validate_top_winner(45, "Class Avg Participation", get_db_comparison_class_avg_participation(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
           ClassBonusData AS (
    ...
    if validate_top_winner(46, "Class Goal Met", get_db_comparison_class_goal_met(),
    ...
    if validate_top_winner(47, "Class All Days Active", get_db_comparison_class_all_days_active(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(48, "Class Goal Met All Days", get_db_comparison_class_goal_met_all_days(),
        """WITH TotalDays AS (SELECT COUNT(DISTINCT log_date) as total_days FROM Daily_Logs),
    ...
    if validate_top_winner(49, "Class Color War Points", get_db_comparison_class_color_war_points(),
